      ...
      <div className="w-full max-w-lg overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        <div className="relative overflow-hidden bg-[#2F5D50] px-6 py-6 text-white">
          ...
          <div className="relative flex items-start justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#D7E3DE]">Contáctanos</p>
              <h2 className="mt-2 text-2xl font-bold">Hagamos realidad tu invitación</h2>
              <p className="mt-2 text-sm leading-6 text-[#D7E3DE]">Cuéntanos un poco sobre lo que necesitas y nos pondremos en contacto contigo.</p>
            </div>
            <button type="button" onClick={onClose} className="ml-4 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/10 text-xl text-white transition hover:bg-white/20">
              ×
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6">
          ...
          {/* PLAN */}
          <div>
            <label className="mb-2 block text-sm font-semibold text-[#263832]">¿Qué plan te interesa?</label>
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setPlan("BASIC")} className={`rounded-xl border px-4 py-3 text-sm font-semibold transition ${plan === "BASIC" ? "border-[#2F5D50] bg-[#2F5D50]/10 text-[#2F5D50]" : "border-[#DCE4DF] bg-white text-[#687A72] hover:bg-[#FAF8F3]"}`}>
                BASIC
              </button>
              <button type="button" onClick={() => setPlan("PRO")} className={`rounded-xl border px-4 py-3 text-sm font-semibold transition ${plan === "PRO" ? "border-[#2F5D50] bg-[#2F5D50]/10 text-[#2F5D50]" : "border-[#DCE4DF] bg-white text-[#687A72] hover:bg-[#FAF8F3]"}`}>
                PRO
              </button>
            </div>
          </div>
