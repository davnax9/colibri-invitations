...
export default function WeddingHero({coverPhoto, details, event}: WeddingHeroProps) {
  return (
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden" style={{ backgroundColor: "var(--theme-primary)" }}>
        {coverPhoto && (<Image src={coverPhoto.url} alt={coverPhoto.title ?? "Fotografía de portada"} fill priority sizes="100vw" className="object-cover"/>)}
        ...
        <div className="relative z-10 px-6 text-center text-white">
            ...
            {details?.phrase && (<p className="mx-auto mt-10 max-w-xl font-serif text-lg italic leading-8 text-white/90 sm:text-xl">"{details.phrase}"</p>)}
