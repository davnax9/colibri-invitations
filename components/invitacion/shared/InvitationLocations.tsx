        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2">
          {event.locations.map((location) => (
            <div key={location.id} className="group rounded-2xl border p-8 text-center transition hover:-translate-y-1 hover:shadow-md" style={{backgroundColor: "var(--theme-background)", borderColor: "var(--theme-accent)"}}>
              ...
              {/* DIRECCIÓN */}
              {location.address ? (
                <p className="mx-auto mt-4 max-w-sm text-sm leading-6" style={{color: "var(--theme-secondary)"}}>{location.address}</p>
              ) : (
              ...
              {/* MAPA */}
              {location.mapsUrl && (
                <a href={location.mapsUrl} target="_blank" rel="noopener noreferrer" className="mt-7 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm font-medium text-white transition hover:opacity-90"
